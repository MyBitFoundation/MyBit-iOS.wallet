...
class TransactionConfigurator {
    ...
    func loadNonce(completion: @escaping (Result<Void, AnyError>) -> Void) {
        session.nonceProvider.getNextNonce(force: true) { [weak self] result in
            switch result {
    ...
    func valueToSend() -> BigInt {
        ...
        if let balance = session.balance?.value, balance == transaction.value {
            if balance == emptyValue {
                value = emptyValue
            } else {
