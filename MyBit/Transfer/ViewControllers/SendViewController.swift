...
class SendViewController: FormViewController {
    ...
    @objc func send() {
        ...
        guard let value = parsedValue else {
            ...
            switch transferType {
            case .ether, .dapp, .nft:
                decimals = Int(log10(Double(EthereumUnit.ether.rawValue)))
            case .token(let token):
                decimals = token.decimals
            }
            let symbot = transferType.symbol(server: session.config.server)
            let message = R.string.localizable.amountDecimalsError(symbot, String(decimals))
            return displayError(string: message)
        }
