...
protocol TokenViewModelDelegate: class {
    func stopRefresh()
}
...
class TokenViewModel {
    ...
    weak var delegate: TokenViewModelDelegate?
    ...
    var imagePlaceholder: UIImage? {
        ...
        guard let contractAddress = MyBitTokenParameters.getContractAddress() else {
            return R.image.ethereum_logo_256()
        }
        
        if token.contractAddress.description.caseInsensitiveCompare(contractAddress) == .orderedSame {
            return R.image.myBitTokenIcon()
        }
    ...
    private func fetchTransactions(page: Int = 0) {
        tokensNetwork.transactions(for: session.account.address, startBlock: 1, page: page, contract: token.contract) { [weak self] result in
