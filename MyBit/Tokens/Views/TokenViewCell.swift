...
class TokenViewCell: UITableViewCell {
    ...
    let titleMinFontSize: CGFloat = 11
    ...
    func configure(viewModel: TokenViewCellViewModel) {
        ...
        let size = viewModel.title.size(withAttributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: titleMinFontSize, weight: .medium)])

        if size.width > titleLabel.bounds.size.width {
            titleLabel.text = viewModel.shortTitle
        } else {
        ...
        symbolImageView.image = viewModel.placeholderImage
