...
struct SetitngsLock: LockInterface {
...
class SettingsViewController: FormViewController, Coordinator {
    ...
    private var lockCoordinator: LockEnterPasscodeCoordinator?
    private var passcodeChangedByUser = true
            ...
            form = Section()
                ...
                <<< SwitchRow(Values.passcodeRow) { [weak self] in
                    ...
                    $0.value = self?.isPasscodeEnabled
                        ...
                        if !self.passcodeChangedByUser {
                            self.passcodeChangedByUser = true
                            return
                        }
                        ...
                        if row.value == true {
                            self.setPasscode { result in
                                if !result {
                                    self.passcodeChangedByUser = false
                                }
                                row.value = result
                                row.updateCell()
                            }
                        } else {
                            self.checkPasscode(completion: { (result) in
                                if result {
    ...
    func checkPasscode(completion: ((Bool) -> Void)? = .none) {
        
        if let lockCoordinator = lockCoordinator {
            lockCoordinator.stop()
        }
        
        let coordinator = LockEnterPasscodeCoordinator(
            model: LockEnterPasscodeViewModel(),
            lock: lock
        )
        
        coordinator.completition = completion
        coordinator.start(showNavBar: true)
        coordinator.showAuthentication(withBioMerick: false, title: R.string.localizable.disableTouchPinTitle())
        
        self.lockCoordinator = coordinator
    }
