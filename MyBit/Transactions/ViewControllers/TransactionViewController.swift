...
class TransactionViewController: UIViewController {
    ...
    init(
        ...
        var addressView = item(title: viewModel.addressTitle, value: viewModel.address)
        if transaction.toAddress == nil {
            let title = NSLocalizedString("transaction.deployContract.label.title", value: "Contract creation", comment: "")
            addressView = titleItem(title: title)
        }
    ...
    private func titleItem(
        title: String
    ) -> UIView {
        return TransactionAppearance.titleItem(title: title)
    }
