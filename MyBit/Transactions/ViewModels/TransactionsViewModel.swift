...
struct TransactionsViewModel {
    ...
    mutating func fetch(page: Int? = 0, completion: (() -> Void)? = .none) {
        if let page = page {
            fetchTransactions(page: page, completion: completion)
        } else {
    ...
    func fetchTransactions(page: Int? = 0, completion: (() -> Void)? = .none) {
        self.network.transactions(for: session.account.address, startBlock: 1, page: page ?? 0, contract: nil) { result in
            if let transactions = result.0 {
                let filteredTransactions = self.filterTransactionByOperations(transactions)
                self.storage.add(filteredTransactions)
                completion?()
            } else {
                completion?()
            }
    ...
    func filterTransactionByOperations(_ transactions: [Transaction]) -> [Transaction] {
        
        var filteredTransactions = [Transaction]()
        for transaction in transactions {
            guard let operation = transaction.operation else {
            ...
            case .tokenTransfer:
                guard let myBitContractAddress = MyBitTokenParameters.getContractAddress(for: config) else { continue }
                guard let operationContractAddress = operation.contract else { continue }
                
                if myBitContractAddress.caseInsensitiveCompare(operationContractAddress) == .orderedSame {
                    filteredTransactions.append(transaction)
                }
            case .unknown:
                filteredTransactions.append(transaction)
