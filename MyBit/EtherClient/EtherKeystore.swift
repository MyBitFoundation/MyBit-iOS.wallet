...
open class EtherKeystore: Keystore {
        ...
        case .keystore(let string, let password):
            importKeystore(
                ...
                case .success(let account):
                    self.filtredWatchAddresses(adress: account.address.description)
    ...
    func filtredWatchAddresses(adress: String)  {
        watchAddresses = watchAddresses.filter { $0 != adress }
    }
