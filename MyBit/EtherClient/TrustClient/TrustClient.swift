...
extension TrustService: TargetType {
    ...
    var task: Task {
        ...
        case .prices(let tokensPrice):
            var symbols = String()
            for token in tokensPrice.tokens {
                symbols.append(token.symbol.uppercased())
                symbols.append(",")
            }
            symbols.removeLast()
            return .requestParameters(parameters: ["currency": tokensPrice.currency, "symbols": symbols], encoding: URLEncoding())
